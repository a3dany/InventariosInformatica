...
<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'tipo', 'error')} required">
    ...
    <g:select id="tipo" name="tipo.id" from="${ii.TipoActivo.list()}" optionKey="id" required=""
              value="${activoInstance?.tipo?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'marca', 'error')} ">
    ...
    <g:select id="marca" name="marca.id" from="${ii.Marca.list()}" optionKey="id" value="${activoInstance?.marca?.id}"
...
<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'estadoActual', 'error')} required">
    ...
    <g:select id="estadoActual" name="estadoActual.id" from="${ii.EstadoActivo.list()}" optionKey="id" required=""
              value="${activoInstance?.estadoActual?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'responsable', 'error')} ">
    ...
    <g:select id="responsable" name="responsable.id" from="${ii.Usuario.list()}" optionKey="id"
              value="${activoInstance?.responsable?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>


<!-- SI PERTENECE A UN ACTIVO-->


<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'perteneceActivo', 'error')} ">
    <label for="perteneceActivo" id="selector1">
...
<div id="control1" class="fieldcontain ${hasErrors(bean: activoInstance, field: 'activoAlQuePertenece', 'error')} ">
    ...
    <g:select id="activoAlQuePertenece" name="activoAlQuePertenece.id" from="${ii.Activo.list()}" optionKey="id"
              value="${activoInstance?.activoAlQuePertenece?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>

<!-- SI ESTA ASIGNADO A UN AMBIENTE-->
...
<div id="control2" class="fieldcontain ${hasErrors(bean: activoInstance, field: 'ambienteActual', 'error')} ">
    ...
    <g:select id="ambienteActual" name="ambienteActual.id" from="${ii.Ambiente.list()}" optionKey="id"
              value="${activoInstance?.ambienteActual?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>



<!-- SI TENEMOS DATOS PARA LA COMPRA -->
...
<div id="controlCompra2"
     class="fieldcontain ${hasErrors(bean: activoInstance, field: 'diasGarantia', 'error')} required">
    <label for="diasGarantia">
        <g:message code="activo.diasGarantia.label" default="Dias Garantia"/>
        <span class="required-indicator">*</span>
    </label>
    <g:field type="number" name="diasGarantia" required=""
             value="${fieldValue(bean: activoInstance, field: 'diasGarantia')}"/>
</div>
...
<div id="controlCompra5" class="fieldcontain ${hasErrors(bean: activoInstance, field: 'fechaCompra', 'error')} ">
    ...
    <joda:dateTimePicker name="fechaCompra" value="${activoInstance?.fechaCompra}" default="${LocalDateTime.now()}"
...
<div class="fieldcontain ${hasErrors(bean: activoInstance, field: 'fechaActivo', 'error')} ">
    ...
    <joda:dateTimePicker name="fechaActivo" value="${activoInstance?.fechaActivo}" default="${LocalDateTime.now()}"
                         noSelection="['': '']"></joda:dateTimePicker>
</div>
